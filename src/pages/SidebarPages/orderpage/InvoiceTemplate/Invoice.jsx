...
import { Toolbar,TextField ,Container,FormControl,Tooltip,Menu, MenuItem, IconButton, Typography,Button,ListItemIcon, ListItemText, OutlinedInput, InputAdornment, colors, Backdrop, CircularProgress } from '@mui/material';
            ...
            <div class="col-12 col-lg-12">
                ...
                <div class="row-invoice">
                        ...
                        <div class="invoice-store-owner-detail-box font-capitalize-case">
                         <h3>{invoiceDetails?.company_name ? invoiceDetails?.company_name : 'Your Company Name'}</h3>
                       <p>{invoiceDetails?.company_address ? invoiceDetails?.company_address : 'Your Company Address'}</p>
                       <p style={{paddingLeft:'10px'}} >{invoiceDetails?.company_state ? invoiceDetails?.company_state : 'Your State'}, {invoiceDetails?.company_zipcode ? invoiceDetails?.company_zipcode : 'Your Zipcode'}</p>
                       <p>{invoiceDetails?.company_phone_number ? invoiceDetails?.company_phone_number : 'Your Phone Number'}</p>
                       </div>
                </div>
      ...
      <Table sx={{ minWidth: 850 }} aria-label="simple table">
        ...
        <TableBody>
            {orderDetail?.products?.map((value,index)=>(
                     ...
                     <TableCell style={{maxWidth:250}} align="left"> {value?.product_name?.toUpperCase()}{value?.product_code ? ` | code: ${value?.product_code}`:''}{(value?.selected_variation && value?.selected_variation[0] ) ? ` | ${value?.selected_variation[0]}`:''}{(value?.selected_variation && value?.selected_variation[1] ) ? ` | ${value?.selected_variation[1]}`:''}</TableCell>
